			     ...
			     struct sde_encoder_virt *sde_enc)
{
	...
	if (ismask != panel->is_hbm_enabled) {
		...
		if (panel->cur_mode->timing.refresh_rate < 120 &&
		    panel->vsync_width) {
			us_per_frame =
				1000000 / panel->cur_mode->timing.refresh_rate;
			diff = diff % us_per_frame;
			if (diff < panel->vsync_width) {
				delay = panel->vsync_width - diff;
			} else {
				rest = us_per_frame - diff;
				delay = rest < REST_TIME ?
						us_per_frame - diff +
							panel->vsync_width :
						0;
			}
			if (delay > 0) {
				SDE_INFO("MSM_LCD code sent delay  %lld us",
					 delay);
	...
	if (isfod != panel->fod_layer) {
		if (isfod) {
			if (ismask) {
				...
				if (panel->in_aod) {
					queue_delayed_work(
						panel->fod_uiready_wq,
						&panel->fod_uiready_delayed_work,
						msecs_to_jiffies(50));
				} else {
					queue_delayed_work(
						panel->fod_uiready_wq,
			...
			panel->fod_layer = isfod;
			queue_delayed_work(panel->fod_uiready_wq,
					   &panel->fod_uiready_delayed_work, 0);
		}
	}
